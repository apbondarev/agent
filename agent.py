...
# Если вкратце, это ваш личный Junior-разработчик, живущий в терминале, мозгом которого выступает нейросеть YandexGPT.
# Вот подробное описание того, как он устроен и что делает:
#
# ### 1. Подключение к «мозгу» (Yandex Cloud)
# Скрипт использует стандартную библиотеку `openai`, но перенаправляет её запросы на серверы Яндекса (`base_url="https://ai.api.cloud.yandex.net/v1"`). Он забирает ваши ключи доступа из переменных окружения или напрямую из кода и подключается к модели YandexGPT Pro.
#
...
# Скрипт спроектирован так, чтобы агент не мог навредить вашей системе. Когда YandexGPT решает, что ей нужно выполнить команду в терминале (например, `rm -rf /` или даже просто `ls`), скрипт ставит процесс на паузу и **спрашивает ваше разрешение** («Разрешить? (y/n)»). Вы можете согласиться, отказаться или даже написать свою версию команды, которую скрипт выполнит вместо предложенной.
#
...
# 2.  **Анализ:** Запрос отправляется в YandexGPT.
...
# 5.  **Возврат результата:** Скрипт незаметно для вас отправляет этот список файлов обратно в YandexGPT со словами: *"Вот результат твоей команды"*.
# 6.  **Ответ:** Получив данные, нейросеть формулирует финальный, понятный ответ и выводит его вам на экран.

# ==========================================
# 1. КОНФИГУРАЦИЯ YANDEX CLOUD
# ==========================================
...
# ==========================================
def main():
    print("🤖 YandexGPT Agent запущен! Введите 'exit' для выхода.")
